...
# | 正常传参场景     | 合法输入下 API 在 backward 中被调用             | 已覆盖：基本调用、NPU tensor、多 callback      |
...
class TestQueueCallbackBasic:
    """验证 queue_callback 在 NPU backward 中的基本调用行为。"""
    ...
    def test_single_callback_executed(self):
        ...
        x = torch.tensor([1.0, 2.0], requires_grad=True, device="npu")
    ...
    def test_callback_with_lambda(self):
        ...
        x = torch.tensor([1.0, 3.0], requires_grad=True, device="npu")
        x.register_hook(hook_fn)
        (x * 2).sum().backward()
    ...
    def test_callback_with_named_function(self):
        """使用具名函数作为 callback。"""
        _require_npu()
        _require_queue_callback()
    ...
    def test_callback_receives_no_args(self):
        ...
        x = torch.tensor([2.0], requires_grad=True, device="npu")
        x.register_hook(hook_fn)
        (x * 3).sum().backward()
...
class TestQueueCallbackMultiple:
    ...
    def test_two_callbacks_both_executed(self):
        ...
        x = torch.tensor([1.0, 2.0, 3.0], requires_grad=True, device="npu")
        x.register_hook(hook_fn)
        x.sum().backward()
    ...
    def test_multiple_callbacks_from_multiple_hooks(self):
...
class TestQueueCallbackWithNPUTensor:
    """验证 queue_callback 与 NPU tensor backward 的协作。"""

    def test_npu_tensor_2d(self):
        """在 2D NPU tensor 的 backward 中使用 queue_callback。"""
        _require_npu()
        _require_queue_callback()
        ...
        x = torch.randn(3, 4, requires_grad=True, device="npu")
        x.register_hook(hook_fn)
        (x * x).sum().backward()

        assert executed["count"] == 1

    def test_callback_can_capture_grad_info(self):
        ...
        def hook_fn(grad):
            # 在 hook 中保存 grad 引用，供 callback 使用
            captured["grad"] = grad
    ...
    def test_callback_callable_object(self):
        ...
        x = torch.tensor([0.5, 1.5], requires_grad=True, device="npu")
        x.register_hook(hook_fn)
        x.sum().backward()

        assert obj.called, "可调用对象应在 backward 后被执行"

    def test_backward_completes_normally(self):
