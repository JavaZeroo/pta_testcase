# 测试目的：验证 Tensor.view API 在 NPU 上的功能行为与接口覆盖
# API 名称：Tensor.view
...
#
# 未覆盖项及原因：
# - TorchScript dtype 视图（文档标注 Warning: not supported by TorchScript，与 NPU 测试无关）
# - 分布式 dtensor 场景（需要分布式环境，不适合单机 NPU 测试）
...
class TestTensorViewShapeBasic:
    ...
    def test_view_1d_to_2d(self):
        """15 → 3x5，多 int 参数形式。"""
        x = torch.randn(15, device=npu_device())
        y = x.view(3, 5)
        assert y.shape == torch.Size([3, 5])
        assert y.device.type == "npu"

    def test_view_2d_reshape(self):
        """4x4 → 2x8，元素数量不变。"""
        x = torch.randn(4, 4, device=npu_device())
        y = x.view(2, 8)
        assert y.shape == torch.Size([2, 8])

    def test_view_4d_to_2d(self):
        """1x2x3x4 → 6x4，高维压缩。"""
        x = torch.randn(1, 2, 3, 4, device=npu_device())
        y = x.view(6, 4)
        assert y.shape == torch.Size([6, 4])

    def test_view_1d_to_4d(self):
        """24 → 1x2x3x4，低维扩展。"""
        x = torch.randn(24, device=npu_device())
        y = x.view(1, 2, 3, 4)
...
class TestTensorViewTorchSize:
    ...
    def test_view_with_torch_size(self):
    ...
    def test_view_with_torch_size_multi_dim(self):
        """torch.Size 多维。"""
        x = torch.randn(24, device=npu_device())
...
class TestTensorViewInferDim:
    ...
    def test_view_infer_first_dim(self):
        """(-1, 5) 推断第一维。"""
        x = torch.randn(15, device=npu_device())
        y = x.view(-1, 5)
        assert y.shape == torch.Size([3, 5])

    def test_view_infer_last_dim(self):
        """(3, -1) 推断最后一维。"""
        x = torch.randn(15, device=npu_device())
        y = x.view(3, -1)
        assert y.shape == torch.Size([3, 5])

    def test_view_infer_single_dim_flatten(self):
        """(-1,) 全展平。"""
        x = torch.randn(3, 4, device=npu_device())
        y = x.view(-1)
        assert y.shape == torch.Size([12])

    def test_view_infer_middle_dim(self):
        """(2, -1, 4) 推断中间维。"""
        x = torch.randn(2, 3, 4, device=npu_device())
        y = x.view(2, -1, 4)
        assert y.shape == torch.Size([2, 3, 4])

    def test_view_infer_with_multi_dim(self):
        """多维中有一个 -1。"""
        x = torch.randn(4, 4, device=npu_device())
        y = x.view(2, -1, 2)
...
class TestTensorViewEmpty:
    ...
    def test_view_empty_tensor_to_1d(self):
        """空 tensor view(0)。"""
        x = torch.empty(0, device=npu_device())
        y = x.view(0)
        assert y.shape == torch.Size([0])
        assert y.device.type == "npu"

    def test_view_empty_tensor_multi_dim(self):
        """空 tensor view 到多维。"""
        x = torch.empty(0, device=npu_device())
        y = x.view(0, 3, 0, 1)
        assert y.shape == torch.Size([0, 3, 0, 1])

    def test_view_empty_multi_dim_back_to_1d(self):
        """多维空 tensor view 回 0。"""
        x = torch.empty(0, device=npu_device())
        y = x.view(0, 3, 0, 1).view(0)
        assert y.shape == torch.Size([0])

    def test_view_empty_infer_zero(self):
        """空 tensor (-1) 推断为 0。"""
        x = torch.empty(0, device=npu_device())
        y = x.view(-1)
        assert y.shape == torch.Size([0])

    def test_view_empty_infer_with_nonzero_dims(self):
...
class TestTensorViewStridedZero:
    ...
    def test_view_stride0_flatten(self):
        """expand 后的 tensor view(-1)。"""
        x = torch.empty(1, 1, device=npu_device()).expand(3, 4)
        contig = x.clone()
        y = x.view(-1)
        expected = contig.view(-1)
        assert y.shape == expected.shape

    def test_view_stride0_add_dims(self):
    ...
    def test_view_stride0_reshape_dims(self):
...
class TestTensorViewDataSharing:
    ...
    def test_view_shares_data(self):
        ...
        t = torch.ones(5, 5, device=npu_device())
        v = t.view(25)
        v[6] = 0.0
        assert t[1, 1].item() == 0.0

    def test_view_data_ptr_same(self):
        """view 与原 tensor 共享 data_ptr。"""
        x = torch.randn(4, 4, device=npu_device())
        y = x.view(16)
        assert x.data_ptr() == y.data_ptr()

    def test_view_chain_shares_data(self):
        """链式 view 共享数据。"""
        x = torch.ones(2, 3, 4, device=npu_device())
        y = x.view(6, 4)
        z = y.view(24)
        z[0] = 99.0
...
class TestTensorViewAs:
    ...
    def test_view_as_basic(self):
        ...
        t = torch.randn(15, device=npu_device())
        template = torch.rand(3, 5, device=npu_device())
        v = t.view_as(template)
        assert v.shape == template.shape
        assert v.device.type == "npu"

    def test_view_as_shares_data(self):
        """view_as 返回的是视图，与原 tensor 共享数据。"""
        t = torch.ones(5, 5, device=npu_device())
        e = torch.empty(25, device=npu_device())
        v = t.view_as(e)
        v[6] = 0.0
        assert t[1, 1].item() == 0.0

    def test_view_as_4d(self):
        """4D 模板 view_as。"""
        t = torch.randn(24, device=npu_device())
        template = torch.empty(2, 3, 4, 1, device=npu_device())
        v = t.view_as(template)
...
class TestTensorViewDtype:
    ...
    def test_view_float32_to_uint8(self):
    ...
    def test_view_float32_to_cfloat(self):
    ...
    def test_view_dtype_data_ptr_same(self):
        """dtype view 共享底层存储（data_ptr 相同）。"""
        x = torch.randn(4, 4, device=npu_device())
        y = x.view(torch.int32)
        assert x.data_ptr() == y.data_ptr()

    def test_view_float32_to_int8(self):
    ...
    def test_view_int8_to_int16(self):
        """int8 → int16（1字节→2字节）。"""
        x = torch.zeros(4, 16, dtype=torch.int8, device=npu_device())
...
class TestTensorViewNonContiguousValid:
    ...
    def test_view_non_contiguous_valid_within_contiguous_chunk(self):
        """非连续 tensor 内合法 view，仅在连续维度块内重排。"""
        tensor = (
            torch.rand(4, 2, 5, 1, 6, 2, 9, 3, device=npu_device())
            .transpose(-1, 2)
            .transpose(-2, 3)
        )
    ...
    def test_view_non_contiguous_add_size1_dims(self):
...
class TestTensorViewRaises:
    ...
    def test_view_incompatible_numel(self):
        """元素数量不匹配，应抛 RuntimeError。"""
        x = torch.randn(15, device=npu_device())
        with pytest.raises(RuntimeError):
            x.view(15, 0)

    def test_view_incompatible_shape(self):
    ...
    def test_view_empty_ambiguous_neg1(self):
        """空 tensor (-1, 0) 无法推断，应抛 RuntimeError。"""
        x = torch.empty(0, device=npu_device())
        with pytest.raises(RuntimeError):
            x.view(-1, 0)

    def test_view_non_contiguous_invalid_crossing(self):
    ...
    def test_view_non_contiguous_invalid_full_flatten(self):
...
class TestTensorViewReturnType:
    ...
    def test_view_returns_tensor(self):
        """view 返回 torch.Tensor 类型。"""
        x = torch.randn(4, 4, device=npu_device())
        y = x.view(16)
        assert isinstance(y, torch.Tensor)

    def test_view_device_preserved(self):
        """view 后设备类型保持 npu。"""
        x = torch.randn(3, 4, device=npu_device())
        y = x.view(12)
        assert y.device.type == "npu"
    ...
    def test_view_dtype_preserved(self):
        ...
        x = torch.randn(4, 4, dtype=torch.float32, device=npu_device())
        y = x.view(16)
        assert y.dtype == torch.float32

    def test_view_is_not_clone(self):
        """view 返回的是视图，不是副本（修改后原 tensor 也变）。"""
        x = torch.ones(4, 4, device=npu_device())
        y = x.view(16)
        y[0] = 999.0
        assert x[0, 0].item() == 999.0

    def test_view_grad_info(self):
        """view 不带 requires_grad 时，返回 tensor 也不带 grad。"""
        x = torch.randn(4, 4, device=npu_device())
        y = x.view(16)
        assert not y.requires_grad

    def test_view_with_requires_grad(self):
        """requires_grad=True 时，view 返回的 tensor 仍携带 grad。"""
        x = torch.randn(4, 4, device=npu_device(), requires_grad=True)
        y = x.view(16)
