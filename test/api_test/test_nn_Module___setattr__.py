# 测试目的：验证 torch.nn.Module.__setattr__ 在 NPU 环境下的功能行为与接口覆盖
...
class TestSetAttrParameter:
    ...
    def test_set_parameter_registers_in_parameters(self):
    ...
    def test_set_parameter_not_in_modules_or_buffers(self):
        device = get_npu_device()
        model = SimpleModule().to(device)
    ...
    def test_set_parameter_device_is_npu(self):
        device = get_npu_device()
        model = SimpleModule().to(device)
        param = nn.Parameter(torch.randn(5, device=device))
        model.npu_param = param
        assert model._parameters["npu_param"].device.type == "npu"

    def test_overwrite_parameter_with_none_deregisters(self):
    ...
    def test_overwrite_parameter_with_new_parameter(self):
...
class TestSetAttrModule:
    ...
    def test_set_submodule_registers_in_modules(self):
        device = get_npu_device()
        model = SimpleModule().to(device)
    ...
    def test_set_submodule_not_in_parameters_or_buffers(self):
        device = get_npu_device()
        model = SimpleModule().to(device)
        sub = SimpleModule().to(device)
        model.encoder = sub
    ...
    def test_overwrite_module_with_new_module(self):
        """将已注册的子模块替换为新模块。"""
        device = get_npu_device()
        model = SimpleModule().to(device)
    ...
    def test_overwrite_module_with_none_deregisters(self):
...
class TestSetAttrBuffer:
    ...
    def test_set_buffer_registers_in_buffers(self):
        ...
        assert model._buffers["my_buf"].device.type == "npu"

    def test_set_buffer_not_in_parameters_or_modules(self):
    ...
    def test_set_buffer_persistent_default(self):
    ...
    def test_set_buffer_non_persistent(self):
    ...
    def test_register_buffer_then_overwrite_with_tensor(self):
        ...
        assert model._buffers["feat"].device.type == "npu"

    def test_register_buffer_then_overwrite_with_none(self):
    ...
    def test_overwrite_buffer_slot_with_non_tensor_non_none_raises_typeerror(self):
...
class TestSetAttrPlainTensor:
    ...
    def test_set_plain_tensor_not_in_parameters(self):
        device = get_npu_device()
        model = SimpleModule().to(device)
    ...
    def test_set_plain_tensor_accessible_as_attr(self):
        device = get_npu_device()
        model = SimpleModule().to(device)
...
class TestSetAttrExceptions:
    ...
    def test_overwrite_param_slot_with_non_param_non_none_raises_typeerror(self):
        """已注册参数槽位赋非 Parameter、非 None 值时，源码明确抛出 TypeError。"""
        device = get_npu_device()
        model = SimpleModule().to(device)
        param = nn.Parameter(torch.randn(3, device=device))
        model.weight = param
        # 将参数槽位赋普通 Tensor（非 Parameter），应抛出 TypeError
        plain_tensor = torch.randn(3, device=device)
        with pytest.raises(TypeError):
            model.weight = plain_tensor

    def test_overwrite_module_slot_with_non_module_non_none_raises_typeerror(self):
...
class TestSetAttrSubclassOverride:
    ...
    def test_subclass_override_with_super_call(self):
        ...
        class PrefixedModule(nn.Module):
            ...
            def __setattr__(self, key, value):
                # 仅对 Parameter 添加前缀，模拟 upstream 测试参考中的用法
